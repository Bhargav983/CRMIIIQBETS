...
const Leads=({navigation})=>{
 ...
 const getUsers = () => {
    fetch('https://iiiqbets.com/CRM-DEMO/admin/view-leads-api.php')
...
useEffect(() => {
  ...
  const getData=()=>{
   ...
   const goReadSheet=(item)=>{
    // alert(item.id);
    console.log("Date 7 Time=",item.date1)
    var str= item.date1;
    var array = str.split(",");
    console.log('Time=',array[1])


     navigation.navigate('Read Sheet',{formname:'Leads',id:item.id,campaign:item.campaign,camp_source:item.camp_source,city:item.city,
      date1:array[0]
    ,time1:array[1],name:item.name,enquiry:item.enquiry,budget:item.budget_range,mobile:item.phone,email:item.email,
  formno:1})
   }

  return(
    <View>
      <Heading myState = 'Welcome to Leads' myColor='blue'/>
